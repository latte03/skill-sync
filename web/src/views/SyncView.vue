...
<template>
  <div class="app-page sync-page">
    <PageHeader eyebrow="远程同步" title="远程同步" summary="连接远程备份，并处理影响同步健康度的变更。"><template #actions><UiButton size="sm" :loading="loading" @click="refresh"><template #icon><n-icon :component="RefreshOutline" /></template>刷新</UiButton></template></PageHeader>
    <n-spin :show="loading"><div v-if="status" class="sync-content">
      <section v-if="needsSetup" class="setup-panel"><header><span class="setup-icon"><n-icon :component="LinkOutline" size="18" /></span><div><p class="meta-label">首次配置</p><h2>建立远程同步</h2><p>完成三个一次性步骤，之后此页只显示同步健康度。</p></div></header><ol><li :class="{ done: status.isRepo }"><span>01</span><div><b>初始化中央仓库</b><small>{{ status.isRepo ? 'Git 仓库已就绪' : '在中央目录创建 Git 仓库' }}</small></div><n-button v-if="!status.isRepo" size="small" type="primary" :loading="initializing" @click="initRepo">初始化</n-button><em v-else>完成</em></li><li :class="{ done: activePlatform?.configured }"><span>02</span><div><b>确认平台凭证</b><small>{{ activePlatform?.configured ? `${activePlatform.name} 已连接` : '在设置中保存 Git Token' }}</small></div><n-button v-if="!activePlatform?.configured" size="small" @click="router.push({ name: 'settings' })"><template #icon><n-icon :component="SettingsOutline" /></template>去设置</n-button><em v-else>完成</em></li><li :class="{ done: status.hasRemote }"><span>03</span><div><b>连接远程仓库</b><small>{{ status.hasRemote ? '远程地址已保存' : '填写 HTTPS 或 SSH 地址' }}</small></div><div v-if="status.isRepo && !status.hasRemote" class="remote-form"><n-input v-model:value="remoteUrl" placeholder="https://github.com/owner/repo.git" /><n-button type="primary" size="small" :loading="savingRemote" @click="setRemote">连接</n-button></div><em v-else-if="status.hasRemote">完成</em></li></ol></section>
      <template v-else>
